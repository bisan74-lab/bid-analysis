...
function computeV2Inference(feature) {
  ...
  // (4) 유사 사례(nearest neighbors): 같은 발주처 우선 → 없으면 업종군에서, 기초금액이 가까운 순 4건.
  //     실제로 일어난 결과(참여수·낙찰 사정률)를 그대로 보여줘 추론의 근거를 투명하게 만든다.
  const pool = orgRows.length >= 4 ? orgRows : grp;
  const 유사사례 = pool
    .map(r => ({ r, d: Math.abs(r.기초금액 - feature.기초금액) }))
    .sort((a, b) => a.d - b.d)
    .slice(0, 4)
    .map(({ r }) => ({
      title: (r.title || '').replace(/\s*\[[^\]]*\]\s*$/, '').replace(/^기초\s*(A값\s*)?(수의|순공사)\s*/, '').slice(0, 32),
      발주처: r.발주처,
      기초금액: r.기초금액,
      참여업체수: r.참여업체수,
      낙찰사정률: r.일순위사정률,
  ...
  // (5) 특이 신호(anomaly): 이 공고가 업종군 통상 대비 두드러지는 점을 짚는다.
  const 특이신호 = [];
  if (grpMed && 예상참여 <= grpMed * 0.6) 특이신호.push({ type: '기회', text: `예상 경쟁(${예상참여}개사)이 업종군 통상(${grpMed}개사)보다 크게 낮음 — 낙찰 여건 유리` });
  if (grpMed && 예상참여 >= grpMed * 1.4) 특이신호.push({ type: '주의', text: `예상 경쟁(${예상참여}개사)이 업종군 통상(${grpMed}개사)보다 크게 높음 — 낙찰 난도 상승` });
  if (발주처낙찰사정률 != null && Math.abs(발주처낙찰사정률 - 그룹낙찰사정률) >= 0.1) {
    const dir = 발주처낙찰사정률 > 그룹낙찰사정률 ? '높은' : '낮은';
